...
Use the following script to recreate the app shortcuts for the startmenu:
 - https://github.com/TheAlienDrew/OS-Scripts/blob/main/Windows/Microsoft-Endpoint-Defender/Recreate-Startmenu-Shortcuts.ps1

If deploying via Intune, please instead using the following script (requires internet access):
 - https://github.com/TheAlienDrew/OS-Scripts/blob/main/Windows/Microsoft-Endpoint-Defender/Recreate-Startmenu-Shortcuts-INTUNE.ps1
...
If some of your users have issues with pins opening up as duplicated icons on the taskbar, look into using this script (resets taskbar, but still requires manual intervention to set the pins back up on their account):
 - https://github.com/TheAlienDrew/OS-Scripts/blob/main/Windows/Microsoft-Endpoint-Defender/Reset-Taskbar-Pins.ps1

If you wish to contribute, either look at the code and add in entries manually, or you can look at [this script I made (Function-Generate-Lnk-Info.ps1)](https://github.com/TheAlienDrew/OS-Scripts/blob/main/Windows/Microsoft-Endpoint-Defender/Function-Generate-Lnk-Info.ps1), that has functions for generating shortcut(s) info in a format compatible with this recreation script.
